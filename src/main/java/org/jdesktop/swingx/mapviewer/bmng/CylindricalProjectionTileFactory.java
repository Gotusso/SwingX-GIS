...
public class CylindricalProjectionTileFactory extends DefaultTileFactory {
    ...
    public Point2D geoToPixel(GeoPosition c, int zoom) {
        ...
        // double size_in_tiles =
        // (double)getInfo().getMapWidthInTilesAtZoom(zoom);
        // double size_in_tiles = Math.pow(2, getInfo().getTotalMapZoom() -
        // zoom);
    ...
    /*
     * x = lat * ppd + fact x - fact = lat * ppd (x - fact)/ppd = lat y =
     * -lat*ppd + fact -(y-fact)/ppd = lat
     */
}
