 ...
 * <h2>Workflow</h2>
 * <p>
 * There are <b>two</b> threads involved in the life cycle of a
 ...
 * thread. {@code BackgroundWorker} invokes the {@link #process process} and
 * {@link #done} events and notifies any {@code PropertyChangeListeners} on this
 * thread. In addition, the {@link #execute} method must be called on this
 * thread
 * </p>
 * </li>
 * <li>
 * <p>
 * <i>Worker</i> thread: The {@link #doInBackground} event is called on this
 * thread. This is where all background activities should happen..</li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * You can retrieve and set progress information on the BackgroundWorker at any
 * time. There are two complementary methods for doing so,
 * <code>setProgress(float)</code> and <code>setProgressPercent(int)</code>. The
 * first method accepts a float between the range of 0 to 1 while the second
 * method accepts an int between the values of 0 and 100.
 * </p>
 * 
 * <p>
 * You may also specify the exact ExecutorService to use for the background
 * task. This allows you to specify a thread pool, default thread priority, and
 * other such attributes of the background task. See the ExecutorService API for
 * more information.
 * </p>
 * 
 * <p>
 * BackgroundWorker operates on the basis of callbacks. You first register a
 * BackgroundListener with the BackgroundWorker, and implement the relevant
 * methods. For example, you implement the <code>doInBackground</code> method if
 * you have tasks that must be performed on the background thread.
 * </p>
 * 
 * <p>
 * From within the <code>doInBackground</code> method you may call the
 * <code>publish</code> method of your BackgroundWorker to place some data on a
 * queue to be processed on the Event Dispatch Thread. At some point the
 * <code>process</code> event will be fired with this data available within the
 * <code>BackgroundEvent</code> object. In this event handler you may freely
 * modify the GUI.
 * </p>
 * 
 * <h2>Sample Usage</h2>
...
public class BackgroundWorker extends AbstractBean {
    ...
    /**
     * True if execute() has been called, and the background task has not yet
     * finished
     */
    ...
    /**
     * @returns true if the BackgroundWorker has finished an execution. If the
     *          BackgroundWorker has not yet been executed, this will return
     *          false. If the BackgroundWorker has been executed, finishes, and
     *          is re-executed then this will be false until the new execution
     *          finishes.
     * 
    ...
    /**
     * @return true if the BackgroundWorker is currently running, false
     *         otherwise
     */
     ...
     * represents 0% and 1 represents 100%. If the progress has changed a
     * percentage point, then a property change event will be fired. Not that
     * changes from 0.1 to 0.11 will not be registered.
     * 
     * This is a convenience method, and is analogous to the setProgressPercent
     * method.
     * 
    ...
    /**
     * Returns the {@code progress} bound property. This method may be called
     * from any thread.
     * 
    ...
    /**
     * Set the progress of this worker based on a value between 0 and 100 where
     * 0 represents 0% and 100 represents 100%. If the progress has changed a
     * percentage point, then a property change event will be fired.
     * 
     * This method may be called from any thread.
     */
    ...
    /**
     * Returns the {@code progressPercent} bound property. This method may be
    ...
    /**
     * Returns the {@code executorService} bound property. This method may be
     * called from any thread.
     * 
    ...
    /**
     * <p>
     * Executes the BackgroundWorker, causing it to begin processing. This
     * method <b>MUST</b> be called on the EDT, and <b>MUST NOT</b> be called if
     * the BackgroundWorker is already running.
     * </p>
     * 
     * <p>
     ...
     * <code>publish</code> method will in turn cause the <code>process</code>
     * event handler to be called on the EDT. From this event handler you are
     * free to update the Swing GUI in any way. If you need to update the
     * <code>progress</code> state, you may do so directly from the
     * <code>doInBackground</code> event handler. Updates to the progress will
     * be set via PropertyChangeEvents on the EDT.</li>
     * <li>Once concluded, the {@code done} event will be called. Subsequent to
     * handling the event, the {@code running} and {@code done} states are
     * updated and PropertyChangeEvents are fired for them if necessary</li>
     * </ol>
     * </p>
     */
    public void execute() {
    ...
    /**
     * Causes the given "chunks" of data to be published on the Event Dispatch
     * Thread. This method causes the <code>process</code> event handler to be
     * called. That event handler is guaranteed to be called on the EDT.
     * 
    ...
    /**
     * Removes the given BackgroundListener such that it will no longer be
     * notified of background events.
     */
    ...
    /**
     * Delegate SwingWorker implementation. BackgroundWorker delegates all of
     * its core tasks to this class. DelegateWorker registers as a
     * PropertyChangeListener on itself so that it can forward any relevant
     * property change events (such as progress changes) on to listeners of
     * BackgroundWorker.
     */
    private final class DelegateWorker extends SwingWorker implements PropertyChangeListener {
        /**
         * Create a new DelegateWorker. This is not intended to be called
         * outside of BackgroundWorker
         */
        private DelegateWorker() {
            // register as a listener of myself to catch SwingWorker property
            // change
            // notifications and propogate them out to listeners of
            // BackgroundWorker
        ...
        /**
         * Necessary because setProgress cannot be overridden or exposed
         * directly
         */
