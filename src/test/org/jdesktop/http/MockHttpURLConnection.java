 ...
 * (which will form the body of the response), the status code that should be
 * returned, and any headers that should be returned.
 * 
 * Note that this version does NOT care about the content-length header set by
 * the client. It really doesn't do anything smart, just trap the data and let
 * you test it.
 * 
 * @author Richard
 */
public class MockHttpURLConnection extends HttpURLConnection {
     ...
     */
    ByteArrayInputStream in = new ByteArrayInputStream("<html><head></head><body>Hello!</body></html>".getBytes());
    /**
    ...
    public String getHeaderField(int n) {
        if (n == 0)
            return "HTTP/1.1 " + status.getCode() + " " + status.getDescription();
        n--;
