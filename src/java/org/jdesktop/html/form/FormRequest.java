...
/**
 * <p>
 * A FormRequest is a Request specifically designed for emulating HTML forms. In
 * particular, a FormRequest does not specifically allow the setting of the body
 * of the Request. Instead, one of several "Form Parameters" are set, which then
 * become the body. Since FormRequest extends request, you can also specify
 * "normal" or "GET" parameters using the setParameter() methods.
 * </p>
 * 
 * <p>
 * When you create a new FormRequest, by default the HTTP method is POST instead
 * of GET (which is the default for Request). In all other ways FormRequest has
 * the same defaults as its parent class.
 * </p>
 * 
 * <p>
 * FormRequest allows you to specify the encoding used for the form. Valid
 * entries are of type {@link Encoding}. The default value is
 * Encoding.UrlEncoding. When uploading files to the server, you would typically
 * want to use Encoding.MultipartFormData.
 * </p>
 * 
 * <p>
 ...
 * multipart/form-data upload with username, compression, and file parameters in
 * the body of the form. Notice the use of the setFormParameter that takes a
 * File. Invoking setFormParameter in this way constructs a FileParameter and
 * sets it on the FormRequest. See {@link FileParameter} for more details.
 * </p>
 * 
 * <p>
 * Since the body of a FormRequest is comprised exclusively of the content of
 * the Form parameters, the various setBody() methods declared in Request are
 * overridden in this subclass to be no-ops. It is therefore only possible to
 * specify the body by using Form parameters.
 * </p>
 * 
 * @author Richard
 */
public class FormRequest extends Request {
    ...
    /**
     * Creates a new FormRequest for the specified URL. It defaults its HTTP
     * method to POST.
     * 
     * @param url
     */
    ...
    /**
     * Creates a Form Parameter using the given key and value and then adds it
     * to the set of form parameters.
     * 
     * @param key
     *            must not be null
     * @param value
    ...
    /**
     * Creates a Form Parameter using the given key and File and then adds it to
    ...
    /**
     * Gets an array of all the Form Parameters for this FormRequest. This array
     * will never be null. Ordering of items is guaranteed based on the order in
     * which the params were added.
     * 
     ...
     * array is treated as an empty array, causing the list of Form params to be
     * removed.
     * 
     * @param params
    ...
    public void setEncoding(Encoding enc) {
        ...
        if (encoding == Encoding.UrlEncoded) {
            ...
            // this should probably be a truly random sequence, though
            // realistically
            // it is probably easier to debug when using a constant seemingly
            // "random"
            // sequence.
            String randomSequence = "7d44e178b043433ff";
            // again, the boundary length (the number of dashes) could be
            // random,
            // but I don't see a strong reason for it. This is probably good
            // enough.
    ...
    protected InputStream getBody() throws Exception {
        ...
        // a MultipartInputStream. This input stream is constructed from a
        // series
        // of "chunks", each chunk being either a String or FormParameter
        // (representing a File to be uploaded). In this way, we have a
        // reference
     ...
     * or with large files. The actual input stream to the files are not opened
     * until necessary.
     */
