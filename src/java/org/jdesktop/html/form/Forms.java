...
/**
 * <p>
 * Utility class used for automatically creating a Form instance directly from
 * an HTTP {@link org.jdesktop.http.Response}. Also contains utility methods for
 * encoding form bodies for transmission, and other Form related functions.
 * </p>
 * 
 * <p>
 * Because HTML is not always well formed, this utility class does not support
 * parsing HTML directly. Rather, it supports parsing HTML documents which have
 * been previously made well-formed. It is then parsed just as any XML document.
 * </p>
 * 
 * <p>
 * Those methods which take a Response directly will take the response text and
 * pass it through some routines to attempt to clean the HTML. If this fails,
 * parsing will fail.
 * </p>
 * 
 * <p>
 * If you want to run your own cleanup routines, do so prior to calling one of
 * the methods which take a SimpleDocument.
 * </p>
 * 
 * @author rbair
 */
public final class Forms {
    ...
    /**
     * Gets a {@link Form} from the given DOM Document, based on the given XPath
     * expression.
     * 
     * @param dom
     *            the DOM Document to parse the HTML Form from.
     * @param baseUrl
     *            the baseUrl which generated this DOM document. This is
     *            required if you later want to submit the HTML Form, since
     *            url's in the Form may be relative.
     * @param expression
     *            the XPath expression to use to find the Form tag to be parsed.
     */
        ...
        try {
            ...
            if (n != null) {
                ...
                for (Node inputNode : dom.getElements(expression + "//input | " + expression + "//select")) {
                    /*
                     * If the input type is "textfield" or "checkbox" or
                     * "password" no need to do anything If the input type is
                     * "radiobutton" collesce it with all other "radiobutton"
                     * types with the same name If the node name is "select"
                     * Save the "name" attribute, and "id" if there is one Dig
                     * out the values for each of the <option> sub tags
                     */
     ...
     *            url's in the Form may be relative.
     * @param name
     *            the name of the Form element within the DOM which should be
     *            parsed
     */
     ...
     *            url's in the Form may be relative.
     * @param index
     *            the index of the Form to parse. For example, if there are 3
     *            &lt;form&gt; tags in the document, and <code>index</code> is
     *            2, then the second form will be selected. <code>index</code>
     *            is 1 based.
     */
     ...
     *            url's in the Form may be relative.
     * @param id
     *            the id of the Form element within the DOM which should be
     *            parsed
     */
    ...
    /**
     * Given a {@link Form}, return the Parameters that would be used to
     * construct an HTTP request for this form.
     * 
     * @return the array of Parameters needed to construct an HTTP request for
     *         this form
     * @param f
     *            the Form to use. If null, an empty parameter array is
     *            returned.
     */
     ...
     * 
     * @param session
     *            the Session to execute this form through. This must not be
     *            null.
     * @return The {@link Response} for this form submition.
     * @throws Exception
     *             if some client error occurs while processing the submition.
     */
    public static Response submit(Form f, Session session) throws Exception {
        ...
        if (!isValidUrl(url)) {
            throw new IllegalStateException("The action url: '" + url
                    + "' does not form a valid url, even when combined with the " + "base url '" + baseUrl + "'");
        }
