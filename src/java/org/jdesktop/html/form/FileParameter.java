...
/**
 * <p>
 * A special {@link Parameter} intended to be used as a Form Parameter on a
 * FormRequest for uploading files.
 * </p>
 * 
 * <p>
 * FileParameter adds the contentType and fileName properties. These properties
 ...
 * then the request will likely fail.
 * </p>
 * 
 * <p>
 * The fileName must be formatted such that creating a new {@link java.io.File}
 * will succeed in locating the file. We use the same rules as the URLConnection
 * class for determining the contentType automatically for the file. In the
 * common case, it will be unnecessary to indicate the contentType manually.
 * However, in special cases you may need to specify the content type manually.
 * The format of the contentType is a mime type, such as text/plain.
 * </p>
 * 
 * <p>
 * If the fileName is correctly specified, then the InputStream returned from
 * getValueStream() will be non-null. You are responsible for closing this
 * stream when finished reading from it.
 * </p>
 * 
 * <p>
 * The "value" of this Parameter is specified to be the fileName. This is
 * enforced by overridding setValue to be a null-op, and by setFilename()
 * calling super.setValue().
 * </p>
 * 
 * @author Richard
 */
public class FileParameter extends Parameter {
     ...
     * parameter (the key, if you will), and the given File as the file to be
     * uploaded by the FormRequest.
     * 
     * @param name
     * @param file
     *            may be null.
     ...
     * 
     * @param contentType
     *            if set to null, then the mime type "content/unknown" is used.
     */
    ...
    /*
     * overridden to disallow clients from setting the value manually. They must
     * set the filename instead.
     */
    @Override
     ...
     * If it is not possible to create such an InputStream due to an incorrect
     * or missing file, then null is returned.
     * 
     * @return
     */
