/*
 * $Id: SimpleHtmlDocument.java 134 2006-12-19 18:39:48Z rbair $
 *
...
/**
 * <p>
 * A DOM {@link org.w3c.dom.Document} that makes it easier to work with DOM
 * documents. This class simply wraps a delegate <code>HTMLDocument</code> and
 * delegates all calls to the Document. This allows this class to work with any
 * HTMLDocument.
 * </p>
 * 
 * @author rbair
...
public class SimpleHtmlDocument implements HTMLDocument {
    ...
    // expressions are saved in SoftReferences, so if memory gets tight they
    // will
    // be released.
    ...
    public SimpleHtmlDocument() {
        this(new SimpleHtmlDocumentBuilder().newPlainDocument());
    }
     ...
     * 
     * @param Document
     *            the HTML DOM document to wrap within this SimpleDocument.
     */
     ...
     * 
     * @return an XML String representation of this DOM
     */
    public String toXML() {
    ...
    /**
     * <p>
     * Returns the child elements of the specified node. This returns only the
     * immediate child Nodes of type ELEMENT_NODE.
     * </p>
     * 
     * @param parent
     *            the parent node
    ...
    public SimpleNodeList getChildElements(Node parent) {
        Node[] nodes = new Node[getChildElementCount(parent)];
        int index = 0;
    ...
    /**
     * Returns the number of child elements for the given node. Only immediate
     * child nodes of type ELEMENT_NODE are counted.
     * 
     * @param parent
     *            the parent node
    ...
    /**
     * Compiles the specified expression, and caches the expression in a
     * SoftReference, so that under stress it can be reclaimed by the system.
     * 
     * @param expression
    ...
    public String getReferrer() {
        return dom.getReferrer();
    }

    public String getDomain() {
        return dom.getDomain();
    }
    ...
    public SimpleHtmlCollection getImages() {
        return new SimpleHtmlCollection(dom.getImages());
    }

    public SimpleHtmlCollection getApplets() {
        return new SimpleHtmlCollection(dom.getApplets());
    }

    public SimpleHtmlCollection getLinks() {
        return new SimpleHtmlCollection(dom.getLinks());
    }

    public SimpleHtmlCollection getForms() {
        return new SimpleHtmlCollection(dom.getForms());
    }

    public SimpleHtmlCollection getAnchors() {
        return new SimpleHtmlCollection(dom.getAnchors());
    }

    public String getCookie() {
        return dom.getCookie();
    }

    public void setCookie(String cookie) {
        dom.setCookie(cookie);
    }
    ...
    public SimpleNodeList getElementsByName(String elementName) {
        return new SimpleNodeList(dom.getElementsByName(elementName));
    }
}
