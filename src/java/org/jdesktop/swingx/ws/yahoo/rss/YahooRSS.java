...
public abstract class YahooRSS extends AbstractBean {
    ...
    /**
     * This method is called while YahooRSS constructs the URL. This prefix MUST
     * NOT contain the trailing "?" or any parameters. YahooRSS will query for
     * those separately and do the work of combining them into a final URL.
     * Hence, the url prefix is everything up to but not including the params
     * 
     * @return a String representing the first part of the URL (including the
     *         http://)
     */
    protected abstract String getUrlPrefix();

    /**
     * @return a Map of parameters used in constructing the URL. The toString()
     *         method will be called on each key and value in order to construct
     *         the param list. They will be URL encoded by YahooRSS, so you need
     *         not worry about url encoding the param ahead of time
     */
     ...
     * reading the feed. Note that this method blocks, and should be called on a
     * background thread.
     * 
     * @return the feed
     * @throws RSSException
     *             if something goes wrong while accessing the RSS feed
     */
