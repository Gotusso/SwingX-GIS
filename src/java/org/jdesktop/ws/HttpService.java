...
public class HttpService extends AbstractHttpService {
    /**
     * The request to use when sending. By default this is a normal Request. If
     * you want to use HTML Form parameters, then use a FormRequest by calling
     * setRequest.
     */
    private Request request = new Request();
    /**
     * The response from the web server. This is reset to null whenever an abort
     * occurs, or at the beginning of a new request/response cycle. Also, when a
     * failure occurs it is set to null. Note that if the request/response cycle
     * succeeds, but the actual call failed (due to semantics or something on
     * the server) "done" will be called and this will have a value.
     */
    ...
    /**
     * Sets the Request to use with this service. If null, then a new Request
     * will be created, ensuring that it is never null. This will fire a
    ...
    /**
     * Gets the response, if there is one. This may return null if the state is
     * anything other than DONE. It will not ever be null if the state is DONE.
     * 
     * @return
     */
    ...
    /**
     * Sets the Response. This simply sets the value and fires a property change
     * event.
     * 
     * @param r
     */
     ...
     * in this class). High security requires that the web server have a
     * certificate signed by a trusted certificate root, such as Verisign. The
     * second level of security is "medium", and is the default for this class.
     * Medium level security means that if a certificate is from a trusted root,
     * then it is trusted. Otherwise, the user is prompted to accept the
     * certificate. This is essentially the mode supported by web browsers. The
     * third level of security is "low" in which case the user will never be
     * prompted. Obviously this is only good for testing (if that) and should
     * never be used when deployed. Note that changing the security level
     * requires priviledges, and thus your app needs to be signed.
     * 
     * @return
     */
    ...
    /**
     * Instances of this class actually perform the request/response cycle in
     * the background. When a task is aborted, "cancel" is called on the worker
     * and then the worker reference becomes null. The worker might still
     * actually be working in the background, but for all intents and purposes
     * is considered to have been garbage collected.
     * 
