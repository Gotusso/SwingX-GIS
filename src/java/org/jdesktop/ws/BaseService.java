...
/**
 * <p>
 * An abstact class useful as a base for asynchronous data service classes. For
 * example, HttpService is a useful subclass for executing REST like webservice
 * calls at a low level. WebService is a useful class which embodies one or more
 * Operations (which are instances of BaseService) and which is useful for
 * executing SOAP requests.
 * </p>
 * 
 * <p>
 * One could also imagine the creation of FtpService, SshService, SftpService,
 * DatabaseService, and other such non http-based services.
 * </p>
 * 
 * <p>
 * BaseService serves as a base class for creating asynchronous Swing-friendly
 * web service APIs. All events fired from BaseService are fired on the EDT, and
 * it is assumed that all methods invoked on BaseService are likewise invoked on
 * the Event Dispatching Thread (EDT).
 * </p>
 * 
 * @author Richard
 */
public abstract class BaseService extends AbstractBean {
     ...
     * quite likely that the service instance will be operating on some
     * background thread (in fact, it is mandatory otherwise this would not be
     * an asynchronous class), it is necessary to push these events coming from
     ...
     * status and then the aborted() method of any AbortListeners will be
     * invoked.
     * 
    ...
    /**
     * The progress of the state. When State is SENDING this is the progress of
     * the upload, or -1 if it is not known. When State is RECEIVING this is the
     * progress of the download, or -1 if it is not known. In the READY and
     * CONNECTING states it is -1 indicating an indeterminate value. In the SENT
     * state it still reflects the amount of data sent. When DONE it represents
     * the amount of data downloaded. When FAILED or ABORTED it is -1 since a
     * reliable value may not be avialable for what it means.
     */
    private float progress;

    /**
     * A list of listeners which are notified whenever the BaseService finishes,
     * aborts, or fails.
     */
     ...
     * EventDispatchThread if necessary. The value of the progress property is
     * set according to the rules outlined in the field javadocs for progress.
     * 
     * @param s
     */
     ...
     * automatically based on the {@link state} of the service. A value of -1
     * indicates that progress is not trackable, either because it is
     ...
     * -1, except for SENT and DONE. This is a float value between 0-1 when it
     * is valid.
     */
    ...
    /**
     * Sets the progress. If values are < -1 or > 1, then they are clamped to -1
     * and 1 respectively. A property change event is fired on the EDT.
     * 
     * @param p
     */
     ...
     * abortable (such as Ready or Done) then nothing happens. The final state
     * may not be set to ABORTED, but the aborted function of any AbortListeners
     * will be invoked.
     */
    ...
    /**
     * Invoke this method from subclasses to force the connection to fail. Note
     * that this only works if the current state is CONNECTING, SENDING, SENT,
     * or RECEIVING. In all other states invoking this method has no effect.
     * 
     * TODO This method should be protected, but the FX-Script class needs
     * access to it. Not sure what the right choice is here. Do not invoke this
     * method directly. In particular, it doesn't abort first, so if the task
     * hasn't already completed then don't even bother calling this method until
     * it does because there will be problems.
     * 
     * @param th
     */
    ...
    /**
     * Invoked by this AbstractHttpService, this method performs the actual send
     * operation. This method must not block.
     * 
    ...
    /**
     * Called by the subclass when the background process completes
     * successfully.
     */
    ...
    /**
     * Fires the given property change event on the EDT in a manner that avoids
     * calling invokeLater for every property change event. This can greatly
     * improve performance in cases where many events happen very quickly.
     * 
     * @param prop
     * @param oldv
     * @param newv
     ...
     * event. It is fired on the EDT. The event is only fired if the number of
     * FailureListeners is > 0. If invoked and the Status is <em>not</em>
     * FAILED, then an IllegalStateException is thrown.
     */
     ...
     * retrieves all events that have occurred in the queue and fires them all
     * off at once. It maintains both the order of the events and also reduces
     * the amount of invokeLater() calls by a substantial amount in some cases.
     */
