...
/**
 * <p>
 * Represents a SOAP web service client. This client is useful for working with
 * SOAP at a lower level, simply packaging the XML body and sending it, and
 * receiving another XML body. No implicit or explicit binding occurs.
 * </p>
 * 
 * <p>
 * This class relies on the availability of the WSDL file. The <code>wsdl</code>
 * property must be set prior to executing an operation. When an operation is
 * executed, the WSDL is parsed and values for port, service, and
 * targetNamespace are determined from the WSDL file if not explicitly specified
 * on the WebService. In practice many WSDL files have only one service and
 * port, so there is little to be gained by specifying them manually since they
 * may be inferred. In the case of a WSDL with multiple ports / services, if not
 * declared, then the first service in the file is used.
 * </p>
 * 
 * @author Richard
 */
public class WebService extends AbstractBean {
    ...
    private String port;
    /**
     * The service description within the WSDL file to use with this WebService
     * instance.
     */
    ...
    /**
     * A map of all the operations that are supported by this webservice. These
     * could be inferred...
     */
    ...
    public static final class Operation extends AbstractHttpService {
        private Document requestDom; // the payload including the name of the
        ...
        private static final class SoapWorker extends SwingWorker<SOAPMessage, SOAPMessage> {
            ...
            protected SOAPMessage doInBackground() throws Exception {
                // NOTE: I had an extreme amount of difficulty getting this to
                // work
                // at all. Originally I did this:
                // SOAPMessage response = dispatch.invoke(msg);
                // to invoke the message, but I was getting odd errors with the
                // weather.gov web service where it wouldn't parse the result.
                // So what
                // I did was bypass this by using Session, Request, and Response
                // and
                // letting the SOAPMessage write out the soap request.
                // I'm sure this isn't robust enough and that I'll need to
                // repent
                // and figure out how to use dispatch.invoke, but it should at
                // least allow
                // me to get some demo's off the ground.

                // for convenience, several values, if null, will be inferred
                // based on
                // the wsdl file at runtime. Specifically, if targetNamespace is
                // null,
                // then the targetNamespace of the <definitions> tag will be
                // used. If
                // port is null, then the first portType listed will be used.
                // Finally,
                // if service is null then the first service listed will be
                // used.

                // NOTE: The following line has been commented out because now
                // that I'm
                // handling the invocation of the SOAP request myself, I must
                // always
                // parse the WSDL
                // boolean mustParseWSDL = targetNamespace == null || port ==
                // null || service == null;
                ...
                String url = null; // Only necessary since I'm invoking the
                                   // service manually
                ...
                if (mustParseWSDL) {
                    ...
                    soapAction = wsdlDom.getString("/definitions/binding/operation[@name='" + op.name
                            + "']/operation/@soapAction");
                }

                // Service s = Service.create(new URL(wsdl), new
                // QName(namespace, serviceName));
                // Dispatch<SOAPMessage> dispatch = s.createDispatch(new
                // QName(namespace, portName), SOAPMessage.class,
                ...
                // depending on what the operation name is, find the operation
                // in the
                // DOM and then look up the SOAPAction for that operation and
                // use it
                if (soapAction != null) {
