...
public class Cookie extends NameValuePair {
    ...
    private int maxAge = -1; // TODO not sure if this is the right way to
    ...
    public static Cookie parseCookie(String s) throws RuntimeException {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (c == '"') {
                inQuotes = !inQuotes;
            } else if (c == '=' && !inQuotes && equals == -1) { // equals==-1
                                                                // exists so
                                                                // that only the
                                                                // first = sign
                                                                // is used as
    ...
    private static void handle(Cookie c, String s, int start, int equals, int end, boolean first)
