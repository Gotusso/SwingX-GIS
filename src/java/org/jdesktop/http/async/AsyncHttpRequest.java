...
/**
 * <p>
 * A simple non visual bean that uses HTTP to interact with web servers. This
 * class conforms (except for XML specific functionality) with XMLHttpRequest as
 * defined in the W3C Working Draft Specification <a
 * href="http://www.w3.org/TR/XMLHttpRequest/"
 * >http://www.w3.org/TR/XMLHttpRequest/</a>.
 * </p>
 * 
 * <p>
 * Several methods, such as the abort() method, cause the AsyncHttpRequest to be
 * "reset". This means that any state associated with the response is cleared,
 * and the readyState reset to UNINITIALIZED.
 * </p>
 * 
 * <p>
 * AsyncHttpRequest relies on {@link org.jdesktop.http.Session} for
 * communicating with the server. It is possible to reuse an existing Session.
 * However, if a Session is not specified, a new one is constructed for the
 * request.
 * </p>
 * 
 * @author rbair
...
public class AsyncHttpRequest extends AbstractBean {
    ...
    public enum ReadyState {
        UNINITIALIZED, OPEN, SENT, RECEIVING, LOADED
    }
    ...
    /**
     * Initializes the AsyncHttpRequest prior to sending a request. Calling this
     * method initializes AsyncHttpRequest by storing the method, uri,
     * asyncFlag, userName, and password values. This also sets the readyState
     * to OPEN, resets the response state to their initial values, and resets
     * the request headers.
     * 
     ...
     * { throw new
     * IllegalStateException("The request must be OPEN before setting the timeout"
     ...
     * Parameter. These are reset whenever this AsyncHttpRequest is reset.
     * 
     * @param name
     ...
     * whenever this AsyncHttpRequest is reset.
     * 
     * @param param
    ...
    /**
     * Adds the given parameter to the set of parameters. These are reset
     * whenever this AsyncHttpRequest is reset. This is a convenience method.
     * 
     * @param name
     ...
     * treated as an empty array. These are reset whenever this AsyncHttpRequest
     * is reset.
     * 
     * @param params
     ...
     * 
     * public AsyncRequestListener[] getAsyncRequestListeners() { return
     * listenerList.getListeners(AsyncRequestListener.class); }
     */
     ...
     * getAsyncRequestListeners()) { l.onLoad(); } }
     * 
     * private void fireOnError() { for (AsyncRequestListener l :
     * getAsyncRequestListeners()) { l.onError(); } }
     * 
     * private void fireOnError() { for (AsyncRequestListener l :
     * getAsyncRequestListeners()) { l.onError(); } }
     * 
     * private void fireOnProgress() { for (AsyncRequestListener l :
     * getAsyncRequestListeners()) { l.onProgress(); } }
     * 
     * private void fireOnAbort() { for (AsyncRequestListener l :
     * getAsyncRequestListeners()) { l.onAbort(); } }
     * 
     * private void fireOnTimeout() { for (AsyncRequestListener l :
     * getAsyncRequestListeners()) { l.onTimeout(); } }
     */
